...
Order::Order(const Order *o) {
    ...
    addDescription();
    setValid(false);
    cout << "hi from order copy constructor\n";
};
...
ostream &operator<<(ostream &out, Order *o) {
    /*
    * Lambda expression: If true return "true", b/c c++ will return 1
    */
 ...
 * then adds reinforcements.
 * 
 */
void Deploy::executeForThisSpecificOrder() {
    if(!pOwnsTerr(p,this->terrToDeploy)){
        ...
        if(getValid()){
            p->subtractReinforcements(numReinToDeploy);
 ...
 * 
 */
void Advance::executeForThisSpecificOrder() {
    if(this->getValid()){ // advance is valid
        if(pOwnsTerr(p,this->terrTarget)){ // player owns target
           if(terrSource->getArmyCount() < numReinToAdvnce){ //check source has enough reinforcements to give to target
...
int Advance::battle() {
    ...
    while (this->terrSource->getArmyCount() > 0 &&
           this->terrTarget->getArmyCount() > 0) { // stop when one Territory has 0 army
 ...
 * 
 */
void Bomb::executeForThisSpecificOrder(){
    if(getValid()){
 ...
 * 
 */
void Blockade::executeForThisSpecificOrder() {
    if(getValid()){
 ...
 * 
 */
void Airlift::executeForThisSpecificOrder() {
    if(getValid()){
 ...
 * 
 */
void Negotiate::executeForThisSpecificOrder() {
    if(getValid()){
    ...
    for (auto pSourceOrder: *pSource->getOrdersList()->getOL()) { // loop through pSource orders list
        if (pSourceOrder->getOrderName().compare("Advance") == 0) { // get Advance orders
            ...
            if (pTarget->ownsTerritory(static_cast<Advance *>(pSourceOrder)->getTerrTarget())) {
                pSource->getOrdersList()->remove(
                        pSourceOrder->getOrderID()); // will remove Advance from ol and delete order ptr
            }
        }
    }
    ...
    for (auto pTargetOrder: *pTarget->getOrdersList()->getOL()) { // loop through pTarget orders list
        if (pTargetOrder->getOrderName().compare("Advance") == 0) { // get Advance orders
            ...
            if (pSource->ownsTerritory(static_cast<Advance *>(pTargetOrder)->getTerrTarget()))
                pTarget->getOrdersList()->remove(
                        pTargetOrder->getOrderID()); // will remove Advance from ol and delete order ptr
        }
    }
}
...
string OrdersList::stringToLog() {
    return "\n----------------------------------------- Logger -----------------------------------------\n OrderList: a new order was added with id " +
           to_string(OL->back()->getOrderID()) +
           "\n------------------------------------------------------------------------------------------\n";
};
