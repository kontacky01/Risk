...
void GameEngine::issueOrdersPhase() {
    for (auto &player: players) {
        ...
        while (continueIssuingOrders) {
            if (player->getStrategyType() == "human") {
                string userInput;
                cout << "\nPlayer " << player->getID() << " (" << player->getStrategyType() << "), would you like to issue an order? (YES/NO): ";
                cin >> userInput;
                transform(userInput.begin(), userInput.end(), userInput.begin(),
                          [](unsigned char c) { return toupper(c); });

                if (userInput == "YES") {
                    string orderName; // get the order name as input or determine it based on the game context
                    cout << "\nEnter an order (bomb, reinforcement, blockade, airlift, diplomacy, deploy, advance): ";
                    cin >> orderName; // assuming user inputs the order name
                    player->issueOrder(orderName);
                ...
                // For computer-controlled players (like AggressivePlayer)
                // Automate the order issuing process based on their strategy
                player->issueOrder("deploy"); // "auto" can be a signal to choose orders automatically based on the strategy
                continueIssuingOrders = false; // Typically, computer players will issue all their orders at once
            }
        }
    ...
    if (ordersVector != nullptr && !ordersVector->empty()) {
        for (int j = 0; j < ordersVector->size(); j++) {
            ...
            if (currentOrder != nullptr && currentOrder->getOrderName() == orderName) {
                cout << "Player " << player->getID() << " is executing " << phase << " order: "
                     << currentOrder->getOrderName() << endl;
